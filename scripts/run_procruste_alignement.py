...
def process_one(comfi_root, sid, task, nb_cams):
    path_to_jcp_mocap = (
        comfi_root / "mocap" / "aligned" / sid / task / "joint_center_positions.csv"
    )  # data from mocap and hpe should be with same unit
    if not path_to_jcp_mocap.exists():
        raise FileNotFoundError(
            f"Missing joint center positions mocap: {path_to_jcp_mocap}"
        )
    ...
    path_to_jcp_hpe = (
        ...
        / sid
        / task
        / f"3d_keypoints_{nb_cams}cams.csv"
    )
    ...
    df_hpe_aligned = pd.DataFrame(
        P_hpe_seq_aligned.reshape(T, 3 * N),
        columns=df_mocap.columns,
        index=df_mocap.index,
    )

    output_csv = (
        Path("output").resolve() / "res_hpe" / sid / task / "3d_keypoints_aligned.csv"
    )
