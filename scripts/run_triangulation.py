...
from comfi_examples.utils import (
    ...
    save_to_csv,
    load_transformation,
    transform_keypoints_list_cam0_to_mocap,
    read_mmpose_scores,
)
...
def process_one(comfi_root: Path, sid: str, task: str, cam_ids: list[int]) -> bool:
    if len(cam_ids) < 2:
        raise ValueError(
            f"At least two cameras are required for triangulation; got {cam_ids}"
        )
    ...
    if missing:
        raise FileNotFoundError(
            "Missing 2D keypoints CSV(s):\n  " + "\n  ".join(missing)
        )
    if not transform_file.exists():
        raise FileNotFoundError(
            f"Missing transform file (cam0->mocap): {transform_file}"
        )
    ...
    filtered = butterworth_filter(
        data=keypoints_in_mocap, cutoff_frequency=10.0, order=5, sampling_frequency=40
    )
