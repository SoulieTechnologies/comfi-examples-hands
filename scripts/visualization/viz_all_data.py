...
from comfi_examples.viz_utils import (
    box_between_frames,
    set_tf,
    draw_table,
    addViewerBox,
...
class Paths:
    ...
    def from_args(
        ...
        # REQUIRED CSVs
        mks_csv = (
            root / "mocap" / split / subject_id / task / "markers_trajectories.csv"
        )
        ...
        if not q_ref_csv.exists():
            raise FileNotFoundError(
                f"Missing joint angles mocap {split} CSV: {q_ref_csv}"
            )
        ...
        jcp_mocap = (
            root / "mocap" / split / subject_id / task / "joint_center_positions.csv"
        )
        ...
        if task in TASKS_WITHOUT_FP:
            ...
            force_data = (
                root / "forces" / split / subject_id / task / f"{task}_devices.csv"
            )
        ...
        if task_has_robot(task):
            ...
            if not robot_csv.exists():
                raise FileNotFoundError(
                    f"CSV not found for robot for task {task} and id {subject_id}: {robot_csv}"
                )
            robot_base = (
                root / "robot" / "robot_in_world" / subject_id / "robot_base_pose.yaml"
            )
            if not robot_base.exists():
                raise FileNotFoundError(
                    f"YAML not found for robot base pose for id {subject_id}: {robot_base}"
                )
        else:
            robot_csv = None
            robot_base = None
...
def parse_args():
    ...
    p.add_argument(
        "--jcp-hpe-mode",
        choices=["aligned", "2cams", "4cams", "3cams"],
        default=None,
...
def define_scene(
    ...
    forceplates_dims_and_centers: Tuple[
    ...
    # Force plates
    R_frame = np.array([[0, -1, 0], [-1, 0, 0], [0, 0, -1]])
    ...
    def cam_frame_name(k):
        return f"f_cam_{k}"

    def cam_text_name(k):
        return f"R_c{k}_text"
    ...
    for k in cam_order:
        T_cam = np.eye(4)
        T_cam[0:3, 0:3] = [[1, 0, 0], [0, 0, -1], [0, 1, 0]]

        Tck = cameras[k]
        meshcat_shapes.frame(
            viz_human.viewer[cam_frame_name(k)],
            axis_length=0.2,
            axis_thickness=0.02,
            opacity=0.8,
            origin_radius=0.02,
        ...
        addViewerBox(
            viz_human, cam_box_name(k), 0.1, 0.1, 0.1, rgba=[0.01, 0.01, 0.01, 1.0]
        )
        set_tf(viz_human, cam_box_name(k), Tck)

        meshcat_shapes.textarea(
            viz_human.viewer[cam_text_name(k)], f"cam{k}", font_size=28
        )
        Ttxt = np.array(Tck)
        Ttxt = Ttxt.copy()
