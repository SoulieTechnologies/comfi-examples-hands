...
class AcadosMPCIKSolver:
    ...
    def _create_ocp_solver(self):
        ...
        W[nmc:nmc + self._nv, nmc:nmc + self._nv] = (
            self._w_dq * np.eye(self._nv)
        )
        W[nmc + self._nv:, nmc + self._nv:] = (
            self._w_u * np.eye(self._nu)
        )
        ocp.cost.W = W

        ocp.cost.yref_e = np.zeros(self._nv)
        ocp.cost.W_e = self._w_dq * np.eye(self._nv)
...
def parse_args():
    ...
    p.add_argument("--w-hand", type=float, default=1.0)
