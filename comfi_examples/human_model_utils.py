...
def get_head_pose(mks_positions):
    ...
    if "Head" in mks_positions:
        ...
        head_center = (mks_positions["RSHO"] + mks_positions["LSHO"]) / 2.0
        top_head = (
            mks_positions["FHD"] + mks_positions["LHD"] + mks_positions["RHD"]
        ) / 3.0
        Y = (top_head - head_center).reshape(3, 1)
        Y = Y / np.linalg.norm(Y)
...
def get_upperarmR_pose(mks_positions):
    ...
    shoulder_center = mks_positions["RSHO"].reshape(3, 1) + (
    ...
    pose[:3, 3] = shoulder_center.reshape(
        3,
    )
...
def get_upperarmL_pose(mks_positions):
    ...
    torso_pose = get_torso_pose(mks_positions)
    bi_acromial_dist = np.linalg.norm(mks_positions["LSHO"] - mks_positions["RSHO"])
    shoulder_center = mks_positions["LSHO"].reshape(3, 1) + (
        torso_pose[:3, :3].reshape(3, 3)
        @ col_vector_3D(0.0, -0.17 * bi_acromial_dist, 0.0)
    ).reshape(3, 1)
...
def get_handR_pose(mks_positions):
    ...
    if "RHL2" in mks_positions:
        wrist_center = (mks_positions["RMWRI"] + mks_positions["RWRI"]).reshape(
            3, 1
        ) / 2.0
        metacarpal_center = (mks_positions["RHL2"] + mks_positions["RHM5"]).reshape(
            3, 1
        ) / 2.0
        ...
        pose[:3, 3] = wrist_center.reshape(
            3,
        )
        pose[:3, :3] = orthogonalize_matrix(pose[:3, :3])
        return pose
    else:
        elbow_center = (mks_positions["RMELB"] + mks_positions["RELB"]).reshape(
            3, 1
        ) / 2.0
        wrist_center = (mks_positions["RMWRI"] + mks_positions["RWRI"]).reshape(
            3, 1
...
def get_handL_pose(mks_positions):
    ...
    if "LHL2" in mks_positions:
        wrist_center = (mks_positions["LMWRI"] + mks_positions["LWRI"]).reshape(
            3, 1
        ) / 2.0
        metacarpal_center = (mks_positions["LHL2"] + mks_positions["LHM5"]).reshape(
            3, 1
        ) / 2.0
        ...
        elbow_center = (mks_positions["LMELB"] + mks_positions["LELB"]).reshape(
            3, 1
        ) / 2.0
        wrist_center = (mks_positions["LMWRI"] + mks_positions["LWRI"]).reshape(
            3, 1
...
def force_vertical_y_rotation(
    R_initial: np.ndarray, y_vertical: np.ndarray
) -> np.ndarray:
    """
...
def get_thorax_pose(mks_positions, gender="male", subject_height=1.80):
    ...
    pose[:3, 3] = (
        (get_pelvis_pose(mks_positions, gender)[:3, 3]).reshape(3, 1) + trans_global
    ).reshape(
        3,
    )
    pose[:3, :3] = orthogonalize_matrix(pose[:3, :3])
...
def get_thighR_pose(mks_positions, gender="male"):
    ...
    hip_center = hip_center + virtual_pelvis_pose[:3, :3].reshape(3, 3) @ col_vector_3D(
        -ratio_x * dist_rPL_lPL, 0.0, 0.0
    )
    hip_center = hip_center + virtual_pelvis_pose[:3, :3].reshape(3, 3) @ col_vector_3D(
        0.0, -ratio_y * dist_rPL_lPL, 0.0
    )
    hip_center = hip_center + virtual_pelvis_pose[:3, :3].reshape(3, 3) @ col_vector_3D(
        0.0, 0.0, ratio_z * dist_rPL_lPL
    )
...
def construct_segments_frames(
    mks_positions, with_hand=True, with_head=True, gender="male", subject_height=1.8
):
    """
...
def get_local_segments_positions(sgts_poses: Dict, with_hand=True) -> Dict:
    ...
    if "thorax" in sgts_poses:
        ...
        local_positions["thorax"] = (
            np.linalg.inv(pelvis_pose) @ thorax_global @ np.array([0, 0, 0, 1])
        )[:3]
    ...
    if "torso" in sgts_poses:
        ...
        thorax_global = sgts_poses["thorax"]
        local_positions["torso"] = (
            np.linalg.inv(thorax_global) @ torso_global @ np.array([0, 0, 0, 1])
        )[:3]
    ...
    if "head" in sgts_poses:
        ...
        torso_global = sgts_poses["torso"]
        local_positions["head"] = (
            np.linalg.inv(thorax_global) @ head_global @ np.array([0, 0, 0, 1])
        )[:3]
    ...
    if "upperarmR" in sgts_poses:
        ...
        torso_global = sgts_poses["torso"]
        local_positions["upperarmR"] = (
            np.linalg.inv(torso_global) @ upperarm_global @ np.array([0, 0, 0, 1])
        )[:3]

    if "upperarmL" in sgts_poses:
        upperarm_global = sgts_poses["upperarmL"]
        torso_global = sgts_poses["torso"]
        local_positions["upperarmL"] = (
            np.linalg.inv(torso_global) @ upperarm_global @ np.array([0, 0, 0, 1])
        )[:3]
    ...
    if "lowerarmR" in sgts_poses:
        ...
        upperarm_global = sgts_poses["upperarmR"]
        local_positions["lowerarmR"] = (
            np.linalg.inv(upperarm_global) @ lowerarm_global @ np.array([0, 0, 0, 1])
        )[:3]

    if "lowerarmL" in sgts_poses:
        lowerarm_global = sgts_poses["lowerarmL"]
        upperarm_global = sgts_poses["upperarmL"]
        local_positions["lowerarmL"] = (
            np.linalg.inv(upperarm_global) @ lowerarm_global @ np.array([0, 0, 0, 1])
        )[:3]
    ...
    if with_hand:
        ...
        if "handR" in sgts_poses:
            ...
            lowerarm_global = sgts_poses["lowerarmR"]
            local_positions["handR"] = (
                np.linalg.inv(lowerarm_global) @ hand_global @ np.array([0, 0, 0, 1])
            )[:3]

        if "handL" in sgts_poses:
            hand_global = sgts_poses["handL"]
            lowerarm_global = sgts_poses["lowerarmL"]
            local_positions["handL"] = (
                np.linalg.inv(lowerarm_global) @ hand_global @ np.array([0, 0, 0, 1])
            )[:3]
    ...
    if "thighR" in sgts_poses:
        ...
        local_positions["thighR"] = (
            np.linalg.inv(pelvis_pose) @ thigh_global @ np.array([0, 0, 0, 1])
        )[:3]

    if "thighL" in sgts_poses:
        thigh_global = sgts_poses["thighL"]
        local_positions["thighL"] = (
            np.linalg.inv(pelvis_pose) @ thigh_global @ np.array([0, 0, 0, 1])
        )[:3]
    ...
    if "shankR" in sgts_poses:
        ...
        thigh_global = sgts_poses["thighR"]
        local_positions["shankR"] = (
            np.linalg.inv(thigh_global) @ shank_global @ np.array([0, 0, 0, 1])
        )[:3]

    if "shankL" in sgts_poses:
        shank_global = sgts_poses["shankL"]
        thigh_global = sgts_poses["thighL"]
        local_positions["shankL"] = (
            np.linalg.inv(thigh_global) @ shank_global @ np.array([0, 0, 0, 1])
        )[:3]
    ...
    if "footR" in sgts_poses:
        ...
        shank_global = sgts_poses["shankR"]
        local_positions["footR"] = (
            np.linalg.inv(shank_global) @ foot_global @ np.array([0, 0, 0, 1])
        )[:3]

    if "footL" in sgts_poses:
        foot_global = sgts_poses["footL"]
        shank_global = sgts_poses["shankL"]
        local_positions["footL"] = (
            np.linalg.inv(shank_global) @ foot_global @ np.array([0, 0, 0, 1])
        )[:3]
...
def get_local_mks_positions(
    sgts_poses: Dict, mks_positions: Dict, sgts_mks_dict: Dict, with_hand=True
) -> Dict:
    ...
    for segment, markers in sgts_mks_dict.items():
        ...
        segment_pose_inv[:3, 3] = (
            -np.transpose(segment_pose[:3, :3]) @ segment_pose[:3, 3]
        )
        for marker in markers:
            if marker in mks_positions:
                # Get the marker's global position
                marker_global_pos = np.append(
                    mks_positions[marker], 1
                )  # Convert to homogeneous coordinates

                marker_local_pos_hom = (
                    segment_pose_inv @ marker_global_pos
                )  # Transform to local coordinates
                marker_local_pos = marker_local_pos_hom[
                    :3
                ]  # Convert back to 3x1 coordinates
